...
const isValidDateValues = (year: number, month: number, day: number): boolean => {
...
export const parseDateStr = (dateStr: string): Date | undefined => {
  ...
  if (parts === undefined || !isValidDateValues(parts.year, parts.month, parts.day)) {
    return undefined;
  }
...
export const isDateInRange = (target: string, start: string, end: string): boolean => {
  const targetDate = parseDateStr(target);
  const startDate = parseDateStr(start);
  const endDate = parseDateStr(end);

  if (targetDate === undefined || startDate === undefined || endDate === undefined) {
    return false;
  }
...
export const getMonthDirNameForDate = (dateStr: string): string => dateStr.slice(0, MONTH_DIR_END);
