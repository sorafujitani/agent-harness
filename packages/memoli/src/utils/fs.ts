...
export const getMonthDirPath = (): string => join(REPORTS_DIR, getMonthDirName());

export const getTodayFilePath = (): string => join(getMonthDirPath(), `${getTodayDateStr()}.md`);

export const getRangeFilePath = (startDate: string, endDate: string): string => {
...
const findMatchingRangeFile = (monthPath: string, targetDate: string): string | undefined => {
  ...
  for (const file of files) {
    ...
    if (parsed !== undefined && isDateInRange(targetDate, parsed.start, parsed.end)) {
...
export const findRangeFileForDate = (targetDate: string): string | undefined => {
  ...
  for (const monthDir of monthDirs) {
    const result = findMatchingRangeFile(join(REPORTS_DIR, monthDir), targetDate);
    if (result !== undefined) {
      return result;
    }
  }
