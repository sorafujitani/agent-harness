...
import { ensureDir, getMonthDirPathForDate, getRangeFilePath, pathExists } from '../utils/fs.ts';
...
const resolveContent = (
  ...
  template === undefined ? `# ${startDate} - ${endDate}\n\n` : loadTemplate(template);
...
const ensureRangeFile = async (ctx: RangeFileContext): Promise<void> => {
  if (await pathExists(ctx.filePath)) {
    ...
    const content = await resolveContent(ctx.startDate, ctx.endDate, ctx.template);
