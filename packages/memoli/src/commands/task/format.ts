...
const OPTIONAL_DETAIL_FIELDS: DetailFieldMapping[] = [
  ...
  {
    label: 'Tags',
    value: (tk) => (tk.tags !== undefined && tk.tags.length > 0 ? tk.tags.join(', ') : undefined),
  },
  { label: 'Memo', value: (tk) => tk.memo },
  {
    label: 'Blocked',
    value: (tk) =>
      tk.blockedBy !== undefined && tk.blockedBy.length > 0 ? tk.blockedBy.join(', ') : undefined,
  },
...
const resolveChildPrefix = (prefix: string, depth: number, isLast: boolean): string => {
  if (depth === 0) {
    return '';
  }
...
const formatTreeNode = (node: TreeNode, prefix: string, isLast: boolean): string[] => {
