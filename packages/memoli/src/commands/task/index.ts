...
import { isTaskStatus, type TaskStatus, type TaskUpdatableFields } from '../../store/types.ts';
import { hasFlag, parseOption } from '../../utils/args.ts';
import { getTodayDateStr } from '../../utils/date.ts';
import { formatTask, formatTaskJson, formatTaskTree, formatTasksJson } from './format.ts';
import { outputTask, outputTaskDetail } from './output.ts';
import { extractTitle, parseDate, parseParentId, parsePriority, parseTags } from './parse.ts';
...
const handleAdd = async (args: string[], json: boolean): Promise<void> => {
  ...
  const parentId =
    parentResult === undefined || parentResult.clear ? undefined : parentResult.value;
...
const buildEditUpdates = (args: string[]): TaskUpdatableFields => {
  ...
  setIfDefined(updates, 'priority', parsePriority(parseOption(args, '--priority')));
  setIfDefined(updates, 'tags', tagStr === undefined ? undefined : parseTags(tagStr));
...
const resolveStatusFilter = (
  ...
  if (statusStr !== undefined) {
    return statusStr.split(',').filter((status): status is TaskStatus => isTaskStatus(status));
  }
  if (!showAll) {
...
const handleList = async (args: string[], json: boolean): Promise<void> => {
  const statusFilter = resolveStatusFilter(parseOption(args, '--status'), hasFlag(args, '--all'));
  const tag = parseOption(args, '--tag');
  const date = parseDate(parseOption(args, '--date'));
  const filter = { status: statusFilter, tag, date };

  await (hasFlag(args, '--tree') ? printTreeOutput(filter, json) : printListOutput(filter, json));
};
...
const isHelpRequest = (args: string[]): boolean => hasFlag(args, '-h') || hasFlag(args, '--help');
