...
const buildSubtree = (task: Task, childrenMap: Map<string, Task[]>, depth: number): TreeNode => ({
  task,
  depth,
...
const collectRoots = (tasks: Task[], childrenMap: Map<string, Task[]>): Task[] => {
  const roots: Task[] = [];
...
const walksToTarget = (taskMap: Map<string, Task>, startId: string, targetId: string): boolean => {
...
export const hasCycle = (tasks: Task[], taskId: string, newParentId: string): boolean => {
