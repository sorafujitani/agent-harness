...
import { buildTree, getDescendants, hasCycle, type TreeNode } from './task-graph.ts';
import {
  TaskStoreSchema,
  type Task,
  type TaskAddOptions,
...
export const loadStore = async (): Promise<TaskStore> => {
  ...
  const result = v.safeParse(TaskStoreSchema, JSON.parse(await readFile(STORE_PATH, 'utf8')));
...
const saveStore = async (store: TaskStore): Promise<void> => {
  ...
  await writeFile(STORE_PATH, `${JSON.stringify(store, undefined, JSON_INDENT)}\n`);
};

const withStoreMutation = async <T>(mutate: (store: TaskStore) => T): Promise<T> => {
...
export const addTask = (title: string, options: TaskAddOptions = {}): Promise<Task> =>
...
const cascadeDescendantsDone = (store: TaskStore, taskId: string, now: string): void => {
...
const revertParentIfDone = (store: TaskStore, parentId: string, now: string): void => {
...
export const updateTaskStatus = (query: string, status: TaskStatus): Promise<Task | undefined> =>
  withStoreMutation((store) => {
    ...
    } else if ((status === 'todo' || status === 'doing') && task.parentId !== undefined) {
...
export const updateTask = (
  ...
  withStoreMutation((store) => {
    ...
    if (updates.parentId !== undefined && hasCycle(store.tasks, task.id, updates.parentId)) {
...
const applyDayScope = (tasks: Task[], date: string): Task[] =>
  tasks.filter((task) => task.scheduledDate === date || task.status === 'doing');
...
const applyFilters = (tasks: Task[], filter: TaskFilter): Task[] => {
  ...
  if (filter.tag !== undefined) {
    ...
    filtered = filtered.filter((task) => task.tags?.includes(filterTag) === true);
  }
...
export const listTaskTree = async (filter: TaskFilter = {}): Promise<TreeNode[]> => {
  const store = await loadStore();
