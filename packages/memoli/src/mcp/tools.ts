...
import { getTodayFilePath, pathExists } from '../utils/fs.ts';
import { asString } from './args.ts';
import { errorResult, jsonResult } from './result.ts';
import { TASK_TOOLS } from './task-tools.ts';
import type { McpCallToolResult, McpToolDefinition, ToolEntry, ToolHandler } from './types.ts';
...
const dailyReadTool: ToolEntry = {
  ...
  handler: async (args) => {
    ...
    const targetPath =
      dateStr === getTodayDateStr() ? filePath : filePath.replace(getTodayDateStr(), dateStr);
    if (!(await pathExists(targetPath))) {
      return errorResult(`Daily file not found for: ${dateStr}`);
    }
    const content = await readFile(targetPath, 'utf8');
    return jsonResult({ path: targetPath, content });
  },
};
...
const ALL_TOOLS: ToolEntry[] = [...TASK_TOOLS, dailyReadTool, memoReadTool, memoListTool];
