...
type ResolveResult<T> = { ok: true; value: T } | { ok: false; error: McpCallToolResult };

export const resolveTaskId = async (query: string): Promise<ResolveResult<string>> => {
...
export const resolveTaskIds = async (queries: string[]): Promise<ResolveResult<string[]>> => {
  const ids: string[] = [];
...
export const resolveAllEdges = async (
  ...
  (await resolveParentEdge(args, updates)) ?? (await resolveBlockedByEdge(args, updates));
