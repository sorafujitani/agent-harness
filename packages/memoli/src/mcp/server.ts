...
const handleToolsCall = async (id: number | string, params: unknown): Promise<void> => {
  const result = v.safeParse(McpCallToolParamsSchema, params);
  if (!result.success) {
    return;
  }
  const toolResult = await dispatchTool(result.output.name, result.output.arguments ?? {});
