  ...
  definition: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        scheduledDate: {
          type: 'string',
          description: 'Day to work on this task (YYYY-MM-DD). Default to today.',
        },
...
const handleTaskUpdate = async (args: Record<string, unknown>): Promise<McpCallToolResult> => {
  const query = resolveQuery(args);
  ...
  definition: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        parent: {
          type: 'string',
          description: 'Move under a parent task (keyword or ID). "none" = root.',
        },
        blockedBy: {
          type: 'array',
          items: { type: 'string' },
